...
import { Plus, Users, School, Loader2 } from 'lucide-react';
...
export const OrganizationSelector = ({ userId, isAdmin, onComplete }: OrganizationSelectorProps) => {
  ...
  return (
    <Card className="w-full max-w-md mx-auto border-white/10 bg-slate-900/80 backdrop-blur-xl shadow-2xl">
      ...
      <CardContent>
        <Tabs defaultValue="join" className="w-full">
          <TabsList className="grid w-full grid-cols-2 rounded-xl h-12 p-1 bg-white/5 border border-white/10">
            <TabsTrigger 
              value="join" 
              className="rounded-lg gap-2 font-bold text-slate-400 data-[state=active]:bg-blue-600 data-[state=active]:text-white"
            >
          ...
          <TabsContent value="join" className="space-y-6 mt-8">
            <div className="space-y-2">
              <Label className="text-xs font-black uppercase text-slate-400 ml-1">Search Schools</Label>
              <Input
                placeholder="Lincoln High School..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="h-12 rounded-xl bg-white/5 border-white/10 text-white placeholder:text-slate-500 focus:ring-blue-600"
              />
            </div>

            <div className="space-y-2">
              <Label className="text-xs font-black uppercase text-slate-400 ml-1">Select School</Label>
                ...
                <SelectContent className="rounded-xl border-white/10 bg-slate-900 text-white">
                  {filteredOrganizations.map(org => (
                    <SelectItem 
                      key={org.id} 
                      value={org.id} 
                      className="h-10 rounded-lg focus:bg-blue-600 focus:text-white"
                    >
                      {org.name}
                    </SelectItem>
                  ))}
                  {filteredOrganizations.length === 0 && (
                    <div className="p-4 text-sm text-slate-500 text-center">No schools found</div>
                  )}
                </SelectContent>
              </Select>
            </div>
            ...
            <Button
              className="w-full h-14 rounded-2xl font-black text-lg bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-600/20 transition-all"
          ...
          {isAdmin && (
            <TabsContent value="create" className="space-y-6 mt-8">
              <div className="space-y-2">
                <Label className="text-xs font-black uppercase text-slate-400 ml-1">New School Name</Label>
                <Input
